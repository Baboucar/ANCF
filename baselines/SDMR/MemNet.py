...
class SDM(nn.Module):
    ...
    def __init__(self,
        ...
        if nonlinear_func == 'relu':
            self._non_linear = F.relu
        elif nonlinear_func == 'tanh':
            self._non_linear = torch.tanh
        else:
    ...
    def forward(self, x, u, j):
        ...
        for i, (A, C) in enumerate(zip(self.A_memories, self.C_memories)):
            ...
            m_a = A._item_embeddings(
                x)  # get the item embeddings in input memory, return batch x seq_len x embedding_size
